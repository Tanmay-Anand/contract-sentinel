...
function fmtTime(epochMicros: number) {
  return new Date(epochMicros / 1000).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })
}
...
export default function TracesPage() {
  ...
  // Build p50 lookup: "serviceName:METHOD:path" → p50Ms
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: "var(--color-text-secondary)" }}>
          Individual API calls received by the services. Click a row to see the full span waterfall.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <select value={sinceMinutes} onChange={(e) => setSinceMinutes(Number(e.target.value))} style={inputStyle}>
        ...
        <input
          type="number" min={0} value={minDurationMs}
      ...
      {traces && traces.length === 0 && (
        ...
        >
          No traces yet — make API calls and Contract Sentinel will receive them within seconds.
        </div>
      )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2.5">
                        {parsed ? (
                          ...
                          <span className="font-mono" style={{ color: "var(--color-text-secondary)" }}>
                            {t.rootName?.slice(0, 16) ?? "—"}
                          </span>
                        )}
                      </td>

                      {/* Path */}
                      <td className="p-2.5 font-mono max-w-xs truncate" style={{ color: "var(--color-text-primary)" }}
                      ...
                      <td className="p-2.5 text-right tabular-nums font-mono"
                        style={{ color: t.hasError ? "var(--color-breaking)" : "var(--color-text-primary)" }}>
                      ...
                      <td className="p-2.5 text-center">
                        <span
                          ...
                          title={p50Ms != null ? `p50 for this endpoint: ${p50Ms.toFixed(1)} ms` : "No baseline yet"}
                        >
                          {ratingStyle.label}
                        </span>
                      </td>
                      ...
                      {/* Spans */}
                      <td className="p-2.5 text-right tabular-nums" style={{ color: "var(--color-text-secondary)" }}>
                        {t.spanCount}
                      </td>

                      {/* Time */}
                      <td className="p-2.5 tabular-nums" style={{ color: "var(--color-text-secondary)" }}>
                      ...
                      <td className="p-2.5">
                        {t.hasError && (
                          <AlertCircle className="w-3.5 h-3.5" style={{ color: "var(--color-breaking)" }} />
                        )}
                      </td>
                    </tr>
                  )
